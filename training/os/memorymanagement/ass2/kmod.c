...
long myIoctl(struct file *fp, unsigned int pid, unsigned long a)
{
	...
	unsigned long int *pt_addr1 = NULL;
	struct page *my_page;
	struct page *my_page2;
	...
	for (i = 765; i < 769; i++) {
		if((my_pgd + i) -> pgd) {
			my_page = (struct page *)(((my_pgd + i) -> pgd) & 0xfffff000);
			//my_page2 =(struct page *)(my_page & 0xfffff000);
			if(i < 768) {
				pt_addr = (unsigned long int *)kmap (my_page);
				for (j = 0; j < 1024; j++) {
					printk (KERN_CONT"%p\t\t%p\t\t%p\n", *(my_pgd + i), (pt_addr + j), *(pt_addr + j));
				}
			}
			else {
	//			pt_addr1 = (unsigned long int *)(((my_pgd + i) -> pgd) + 0xc0000000);
				my_page = (struct page *)(((unsigned int)my_pgd + i) & 0xfffff000);
				pt_addr1 = (unsigned long int *)kmap (my_page);
				for (j = 0; j < 1024; j++) 
					printk (KERN_CONT"%p\t\t%p\t\t%p\n", *(my_pgd + i), pt_addr1, *(pt_addr1 + j) );
			}
		}
	}
	return 0;
